//! # lemon-mint
//!
//! A pure-Rust port of the famous [Lemon parser generator](https://www.hwaci.com/sw/lemon/),
//! packaged as a **library with an API** instead of a command-line tool.
//!
//! You describe an LALR(1) grammar — either as a Lemon-style `.y` grammar string/file, or by
//! calling builder methods — and lemon-mint emits a self-contained Rust source file containing the
//! parser's state-machine tables and a small driver. The generated parser is plain Rust with **no
//! runtime dependency on this crate**, so you can commit it to your repository or generate it from a
...
//! ## The three steps
//!
//! 1. Build a grammar with [`LemonMintBuilder`]. Either feed it a whole grammar with
//!    [`load_y`](LemonMintBuilder::load_y) / [`load_y_file`](LemonMintBuilder::load_y_file), or
//!    assemble it piece by piece with [`set_start_symbol`](LemonMintBuilder::set_start_symbol),
//!    [`set_token_type`](LemonMintBuilder::set_token_type), [`add_type`](LemonMintBuilder::add_type),
//!    [`add_rule`](LemonMintBuilder::add_rule), and friends. The two styles can be mixed freely.
//! 2. Compile the grammar into the parser tables with
//!    [`try_into_lemon`](LemonMintBuilder::try_into_lemon), which returns a [`LemonMint`]. Grammar
//!    errors and unresolved parser conflicts are reported here as an [`Err`].
//! 3. Emit Rust source with [`gen_rust`](LemonMint::gen_rust). Optionally write a human-readable
//!    report of the state machine with [`gen_log`](LemonMint::gen_log) (the classic Lemon
//!    `y.output`), or a normalized copy of the grammar with [`gen_y`](LemonMint::gen_y).
//!
//! ## Example: a calculator
//!
//! This generates a parser for newline-separated arithmetic expressions, then runs it on
//! `15 / 5` to get `3.0`:
//!
//! ```
//! use std::sync::Arc;
...
//!
//! [`gen_rust`](LemonMint::gen_rust) wraps everything in a `code` module. The two items you use from
//! it are `Token` (an enum with one variant per terminal symbol) and `Parser`:
//!
//! ```ignore
//! use code::{Parser, Token};
//!
...
//! complete sentence of the grammar. The whole driver lives in `code`; a second `rules` module holds
//! the action code and is not used directly. Items from your crate are reachable from actions and
//! `%code` as `super::…` or `crate::…` (e.g. `super::Expr`).
//!
//! ### Soft keywords with `try_add_token`
//!
//! `Parser::try_add_token` is a non-committal variant of `add_token`: it returns `Ok(true)` if the
//! token is accepted in the current state, or `Ok(false)` if it is not — **without** raising a syntax
//! error. This lets a tokenizer treat a word as a keyword where the grammar expects one and fall back
//! to feeding it as an identifier elsewhere ("soft" / non-reserved keywords):
//!
//! ```ignore
...
//! ## Y-grammar reference
//!
//! lemon-mint accepts a grammar syntax close to [classic Lemon](https://www.hwaci.com/sw/lemon/).
//! Line (`//`) and block (`/* … */`) C-style comments are allowed anywhere.
//!
//! ### Rules and actions
//!
//! A rule is `Lhs ::= Rhs.` optionally followed by an action — Rust code producing the semantic value
...
//! action. A rule with no action produces `()`. Parenthesized names like `(a)` are **aliases** that
//! bind the semantic value of that symbol to a variable usable in the action. The action also has
//! `&mut extra` in scope (the `%extra_argument`).
//!
//! ### Terminals vs nonterminals
//!
//! A symbol whose name contains at least one lowercase letter is a **nonterminal**; an all-uppercase
//! name is a **terminal** (a `Token`). (Classic Lemon decides this from the first letter only.)
//!
//! ### Types
//!
//! Every symbol carries a semantic value. Terminals all share the `%token_type`. Each nonterminal's
//! type is set with `%type Name {Type}`, or defaults to `%default_type`. The start symbol's type is
//! what the generated `Parser::end` returns.
//!
//! ### Precedence and associativity
//!
//! `%left`, `%right`, and `%nonassoc` declare terminal precedence to resolve shift/reduce conflicts.
//! Each successive declaration binds more tightly than the previous one:
//!
//! ```text
...
//!
//! Directive syntax is permissive — braces, a trailing `.`, or a bare token all work, and a braced
//! value may span multiple lines up to its matching closing brace:
//!
//! ```text
//! %start_symbol {Unit}
//! %start_symbol Unit.
...
//!
//! * A symbol is a nonterminal if its name contains any lowercase letter (classic Lemon looks only at
//!   the first letter).
...
'l:		while let Some(line) = lines.next()
	...
	pub fn get_type(&self, mut symbol_name: &str) -> Option<(Arc<String>, usize)>
	{	symbol_name = symbol_name.trim();
		self.nonterminal_types.get(symbol_name).map(|v| (Arc::clone(&v.filename), v.n_line))
	}
	...
	///	Add one grammar rule, equivalent to a `Lhs ::= Rhs. action` line in a Y-grammar.
	///
	/// * `lhs_name` - left-hand side nonterminal, e.g. `"Exprs"`.
	/// * `rhs_names_aliases` - the right-hand side: whitespace-separated symbol names, each optionally
	///   followed by an alias in parentheses to bind its semantic value, e.g. `"Exprs(items) COMMA Expr(item)"`.
	///   An empty string is an empty production.
	/// * `code` - the action: Rust code producing the left-hand side's value (it may use the aliases
	///   and `&mut extra`). An empty string produces `()`.
	///
	/// # Example
	///
	/// ```
	/// # use lemon_mint::LemonMintBuilder;
	/// # use std::sync::Arc;
	/// let fake_filename = Arc::new("source.y".to_string());
	/// let builder = LemonMintBuilder::new()
	/// 	.set_default_type("Vec<f64>".to_string()).unwrap()
	/// 	.add_rule(&fake_filename, 1, "List".to_string(), "List(items) NUM(n)", "let mut items = items; items.push(n); items".to_string()).unwrap()
	/// 	.add_rule(&fake_filename, 2, "List".to_string(), "", "Vec::new()".to_string()).unwrap();
	/// ```
...
'l:				while let Some(mut c) = it.next()
	...
	///
	/// * `basisflag` - print only the basis configurations of each state, not the full closure.
	/// * `show_precedence_conflict` - also report conflicts that were resolved by precedence rules.
	...
	/// Write a normalized copy of the grammar to `out`: the list of symbols and the rules, without
	/// comments or action code. Handy when the grammar was assembled through builder calls and you
	/// want to see the equivalent Y-grammar.
